...
from database import BirdNetSimpleDB
...
def safe_print(text: str, **kwargs):
    ...
    try:
        ...
        print(f"{safe_text} (contains non-ASCII characters)", **kwargs)


def import_results_from_directory(directory_path: str, session_name: str = None, interactive: bool = False, model_name: str = "BirdNET", model_type: str = "default") -> dict:
    ...
    if session_name is None:
        if interactive:
            ...
            safe_print(f"[INFO] 自動生成されたセッション名: {session_name}")
    ...
    safe_print(f"Importing {len(csv_files)} CSV files into session '{session_name}' with model '{model_name}' (type: {model_type})...")
    
    for csv_file in csv_files:
        safe_print(f"Processing: {csv_file.name}")
        
        try:
            import_result = db.import_csv_results(str(csv_file), session_name, model_name, model_type)
            
            if import_result['success']:
                results['imported_files'] += 1
                results['total_detections'] += import_result['detections_imported']
                audio_files_count = import_result.get('audio_files_found', 1)
                safe_print(f"  [OK] Imported {import_result['detections_imported']} detections from {audio_files_count} audio file(s)")
            else:
                results['failed_files'] += 1
                safe_print(f"  [ERROR] Failed: {import_result.get('error', 'Unknown error')}")
    ...
    safe_print("\nImport completed:")
    safe_print(f"  Session: '{session_name}'")
    safe_print(f"  Files: {results['imported_files']}/{results['total_files']} successful")
    safe_print(f"  Detections: {results['total_detections']} total")
    
    # セッション名の解析結果を表示
    manager = LocationSpeciesDateManager()
    parsed = manager.parse_session_name(session_name)
    if parsed['valid']:
        safe_print(f"  [INFO] 場所: {parsed['location']}")
        safe_print(f"  [INFO] 種名: {', '.join(parsed['species'])}")
        safe_print(f"  [INFO] 日付: {parsed['date']}")
...
def import_single_file(csv_file_path: str, session_name: str = None, interactive: bool = False) -> dict:
    ...
    safe_print(f"Importing {csv_path.name} into session '{session_name}'...")
    
    try:
        import_result = db.import_csv_results(str(csv_path), session_name)
        
        if import_result['success']:
            safe_print(f"[OK] Successfully imported {import_result['detections_imported']} detections")
        else:
            safe_print(f"[ERROR] Import failed: {import_result.get('error', 'Unknown error')}")
...
def show_sessions():
    ...
    for session in sessions:
        ...
        if session['location']:
            print(f"  [INFO] 場所: {session['location']}")
        if session['species']:
            print(f"  [INFO] 種名: {session['species']}")
        if session['analysis_date']:
            print(f"  [INFO] 日付: {session['analysis_date']}")
        print(f"  Files: {session['file_count']}, Detections: {session['detection_count']}")
        print(f"  Avg Confidence: {session['avg_confidence']:.3f}" if session['avg_confidence'] else "  Avg Confidence: N/A")
        print(f"  Created: {session['first_created']} - {session['last_created']}")
        print("-" * 80)
...
def show_stats():
    ...
    if stats['top_species']:
        ...
        for species in stats['top_species'][:5]:
            safe_print(f"  {species['common_name']}: {species['detection_count']} detections (avg: {species['avg_confidence']:.3f})")
    else:
...
def show_session_name_help():
    ...
    print("[EXAMPLE] 例: 奥多摩_ヨタカ_20240629")
    print()
    print("[USAGE] 使用方法:")
    ...
    print()
    print("[EXAMPLES] 使用例:")
    print("  python lib/import_results_simple.py database/analysis_results --interactive")
    print("  python lib/import_results_simple.py database/analysis_results --location '森林公園' --species 'ヨタカ'")
    print("  python lib/import_results_simple.py database/analysis_results --session '奥多摩_ヨタカ_20240629'")


def main():
    parser = argparse.ArgumentParser(description='Import BirdNET CSV results to simple database')
    ...
    parser.add_argument('--model', '-m', default='BirdNET', help='Model name used for analysis')
    parser.add_argument('--model-type', default='default', help='Model type: default or custom')
    ...
    if args.export:
        ...
        if db.export_to_csv(args.export, session_name):
    ...
    # pathが必要な場合のチェック
    if not args.path:
        print("Error: path argument is required when not using --list, --stats, --export, or --help-naming")
        parser.print_help()
        sys.exit(1)
    ...
    if path.is_file():
        ...
        model_type = getattr(args, 'model_type', 'default')  # --model-type は model_type としてアクセス
        result = import_results_from_directory(str(path), args.session, args.interactive, args.model, model_type)
    else:
