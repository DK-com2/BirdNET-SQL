...
def import_results_from_directory(directory_path: str, session_name: str = None, interactive: bool = False) -> dict:
    ...
    for csv_file in csv_files:
        ...
        try:
            import_result = db.import_csv_results(str(csv_file), session_name)
...
def show_stats():
    ...
    if stats['top_species']:
        ...
        for species in stats['top_species'][:5]:
            print(f"  {species['common_name']}: {species['detection_count']} detections (avg: {species['avg_confidence']:.3f})")
    else:
