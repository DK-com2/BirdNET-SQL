...
def loadModel(class_output=True):
    ...
    cfg.MODEL_PATH = f"{PROJECT_PATH}/lib/birdnet/checkpoints/V2.4/BirdNET_GLOBAL_6K_V2.4_Model_FP32.tflite"

    # Do we have to load the tflite or protobuf model?
    if cfg.MODEL_PATH.endswith(".tflite"):
        # Load TFLite model and allocate tensors.
        INTERPRETER = tflite.Interpreter(
            model_path=cfg.MODEL_PATH, num_threads=cfg.TFLITE_THREADS
        )
...
def loadCustomClassifier():
    ...
    if cfg.CUSTOM_CLASSIFIER.endswith(".tflite"):
        ...
        C_INTERPRETER = tflite.Interpreter(
            model_path=cfg.CUSTOM_CLASSIFIER, num_threads=cfg.TFLITE_THREADS
        )
...
def loadMetaModel():
    ...
    M_INTERPRETER = tflite.Interpreter(
        model_path=cfg.MDATA_MODEL_PATH, num_threads=cfg.TFLITE_THREADS
    )
...
def trainLinearClassifier(
    ...
    if not cfg.MULTI_LABEL:
        ...
        x_train, y_train, x_val, y_val = utils.random_multilabel_split(
    ...
    if upsampling_ratio > 0:
        x_train, y_train = utils.upsampling(
            x_train, y_train, upsampling_ratio, upsampling_mode
        )
    ...
    lr_schedule = keras.experimental.CosineDecay(
        learning_rate, epochs * x_train.shape[0] / batch_size
    )
...
def saveLinearClassifier(
    classifier, model_path: str, labels: list[str], mode="replace"
):
    ...
    cfg.PB_MODEL = (
        f"{PROJECT_PATH}/lib/birdnet/checkpoints/V2.4/BirdNET_GLOBAL_6K_V2.4_Model"
    )
    ...
    if mode == "replace":
        combined_model = tf.keras.Sequential(
            [saved_model.embeddings_model, classifier], "basic"
        )
    elif mode == "append":
        intermediate = classifier(saved_model.model.get_layer("GLOBAL_AVG_POOL").output)

        output = tf.keras.layers.concatenate(
            [saved_model.model.output, intermediate], name="combined_output"
        )
...
def custom_loss(y_true, y_pred, epsilon=1e-7):
    ...
    positive_loss = -K.sum(
        y_true * K.log(K.clip(y_pred, epsilon, 1.0 - epsilon)), axis=-1
    )

    # Calculate loss for negative labels with epsilon
    negative_loss = -K.sum(
        (1 - y_true) * K.log(K.clip(1 - y_pred, epsilon, 1.0 - epsilon)), axis=-1
    )
...
def predict(sample):
    ...
    if PBMODEL == None:
        ...
        INTERPRETER.resize_tensor_input(
            INPUT_LAYER_INDEX, [len(sample), *sample[0].shape]
        )
        INTERPRETER.allocate_tensors()
...
def predictWithCustomClassifier(sample):
    ...
    if C_PBMODEL == None:
        ...
        C_INTERPRETER.resize_tensor_input(
            C_INPUT_LAYER_INDEX, [len(vector), *vector[0].shape]
        )
        C_INTERPRETER.allocate_tensors()
