        ...
        for timestamp in getSortedTimestamps(r):
            ...
            for c in r[timestamp]:
                ...
                formatted_start_time = (
                    f"{int(float(start)//60)}m{int(float(start)%60)}s"
                )
                formatted_end_time = f"{int(float(end)//60)}m{int(float(end)%60)}s"
...
def safe_print_filename(fpath: str, prefix: str = "", suffix: str = "") -> None:
    """日本語ファイル名を安全に表示する"""
    try:
        print(f"{prefix}{fpath}{suffix}", flush=True)
    except UnicodeEncodeError:
        # ファイル名をASCIIで表示
        safe_name = os.path.basename(fpath).encode('ascii', 'replace').decode('ascii')
        print(f"{prefix}{safe_name} (original name contains non-ASCII characters){suffix}", flush=True)
...
def analyzeFile(item):
    ...
    # Status - 日本語ファイル名対応
    safe_print_filename(fpath, "Analyzing ")
    ...
    try:
        ...
        # Write error log
        safe_print_filename(fpath, "Error: Cannot analyze audio file ", ".\n")
        utils.writeErrorLog(ex)
    ...
    try:
        ...
        # Write error log
        safe_print_filename(fpath, "Error: Cannot save result for ", ".\n")
        utils.writeErrorLog(ex)

        return False

    delta_time = (datetime.datetime.now() - start_time).total_seconds()
    safe_print_filename(fpath, "Finished ", f" in {delta_time:.2f} seconds")
