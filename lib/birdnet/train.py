...
# cfg.TRAIN_DATA_PATH = "data/audio/train/"
# cfg.SAMPLE_CROP_MODE = "segments"
# cfg.SIG_OVERLAP = 2.0
# cfg.CUSTOM_CLASSIFIER = "model/default/MODEL"
...
def _loadTrainingData(cache_mode="none", cache_file="", progress_callback=None):
    ...
    if cache_mode == "load":
        if os.path.isfile(cache_file):
            ...
            x_train, y_train, labels, cfg.BINARY_CLASSIFICATION, cfg.MULTI_LABEL = (
                utils.loadFromCache(cache_file)
            )
    ...
    valid_labels = [
        l
        for l in labels
        if not l.lower() in cfg.NON_EVENT_CLASSES and not l.startswith("-")
    ]
    ...
    if cfg.BINARY_CLASSIFICATION and cfg.MULTI_LABEL:
        raise Exception(
            "Error: Binary classfication and multi-label not possible at the same time"
        )
    ...
    for folder in folders:
        ...
        files = filter(
            ...
            (
                ...
                if not f.startswith(".")
                and f.rsplit(".", 1)[-1].lower() in cfg.ALLOWED_FILETYPES
            ),
        )
...
def trainModel(on_epoch_end=None, on_trial_result=None, on_data_load_end=None):
    ...
    print(
        f"...Done. Loaded {x_train.shape[0]} training samples and {y_train.shape[1]} labels.",
        flush=True,
    )
        ...
        class BirdNetTuner(keras_tuner.BayesianOptimization):
            ...
            def run_trial(self, trial, *args, **kwargs):
                ...
                for execution in range(int(self.executions_per_trial)):
                    print(
                        f"Running Trial #{trial_number} execution #{execution + 1}",
                        flush=True,
                    )
                    ...
                    classifier, history = model.trainLinearClassifier(
                        ...
                        train_with_mixup=hp.Boolean(
                            "mixup", default=cfg.TRAIN_WITH_MIXUP
                        ),
    ...
    best_val_auroc = history.history["val_AUROC"][
        np.argmin(history.history["val_loss"])
    ]
    ...
    if cfg.TRAINED_MODEL_OUTPUT_FORMAT == "both":
        ...
        model.saveLinearClassifier(
            classifier, cfg.CUSTOM_CLASSIFIER, labels, mode=cfg.TRAINED_MODEL_SAVE_MODE
        )
        ...
        raise ValueError(
            f"Unknown model output format: {cfg.TRAINED_MODEL_OUTPUT_FORMAT}"
        )

    print(
        f"...Done. Best AUPRC: {best_val_auprc}, Best AUROC: {best_val_auroc}",
        flush=True,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=32, help="Batch size. Defaults to 32."
    )
    parser.add_argument(
        "--val_split",
        type=float,
        default=0.2,
        help="Validation split ratio. Defaults to 0.2.",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
    ...
    parser.add_argument(
        "--dropout", type=float, default=0.0, help="Dropout rate. Defaults to 0."
    )
    parser.add_argument(
        "--mixup",
    ...
    parser.add_argument(
        "--cache_mode",
        default="none",
        help="Cache mode. Can be 'none', 'load' or 'save'. Defaults to 'none'.",
    )
    parser.add_argument(
        "--cache_file",
        default="train_cache.npz",
        help="Path to cache file. Defaults to 'train_cache.npz'.",
    )
    parser.add_argument(
        "--threads",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Minimum frequency for bandpass filter in Hz. Defaults to {} Hz.".format(
            cfg.SIG_FMIN
        ),
    )
    parser.add_argument(
        ...
        help="Maximum frequency for bandpass filter in Hz. Defaults to {} Hz.".format(
            cfg.SIG_FMAX
        ),
    )
    ...
    cfg.TRAIN_WITH_MIXUP = (
        args.mixup if args.mixup is not None else cfg.TRAIN_WITH_MIXUP
    )
    ...
    # Train model
    print("-----EPOCHS------", cfg.TRAIN_EPOCHS)
    print("-----AUTOTUNE------", cfg.AUTOTUNE)
    print("-----AUTOTUNE_TRIALS------", cfg.AUTOTUNE_TRIALS)
    print("-----FMIN------------", args.fmin)
    trainModel()
