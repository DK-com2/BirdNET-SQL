...
本ツールは高速化のために以下の設定で実行されます：

### 🚀 最適化設定

| 設定項目 | 設定値 | 効果 |
|------------|-----------|------|
| **並列処理スレッド** | 12 | 複数ファイルを同時処理 |
| **信頼度閾値** | 0.25 | ノイズを除外して高速化 |

### 🔧 カスタマイズ

設定を変更したい場合は、`start_analysis.py`の`run_analysis`メソッドで調整できます：

```python
# 信頼度閾値の調整例
"--min_conf", "0.1"   # より高精度（やや低速）
"--min_conf", "0.5"   # より高速（やや低精度）

# スレッド数の調整例
"--threads", "8"      # より安全（やや低速）
"--threads", "16"     # 最大パフォーマンス（システム負荷高）
```
